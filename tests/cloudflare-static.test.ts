...
test("cloudflare setup scripts document repeatable deployment", async () => {
  ...
  const demoWrangler = await readFile("wrangler.demo.toml", "utf8");
  const readme = await readFile("README.md", "utf8");

  assert.equal(packageJson.scripts["cf:setup"], "node scripts/cf-setup.mjs");
  assert.equal(packageJson.scripts["cf:deploy-demo"], "tsc -p tsconfig.json && vite build && node scripts/run-wrangler.mjs deploy --config wrangler.demo.toml");
  assert.equal(packageJson.scripts["cf:migrate"], undefined);
  assert.equal(packageJson.scripts["cf:migrate:local"], undefined);
  ...
  assert.doesNotMatch(setupScript, /npx wrangler/);
  assert.match(setupScript, /yarn cf:release/);
  ...
  assert.doesNotMatch(setupScript, /PREVIEW_DATABASE_NAME/);
  assert.match(setupScript, /--force/);
  assert.match(setupScript, /<your-d1-database-id>/);
  assert.doesNotMatch(setupScript, /<your-preview-d1-database-id>/);
  assert.match(demoWrangler, /name = "recall-inbox-demo"/);
  assert.match(demoWrangler, /DEMO_MODE = "true"/);
  assert.doesNotMatch(demoWrangler, /d1_databases/);
  assert.match(readme, /yarn cf:setup/);
