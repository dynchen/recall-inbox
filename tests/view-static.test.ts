...
test("review page exposes protected manual sync controls", async () => {
  ...
  assert.match(app, /Enter ADMIN_SECRET to unlock items\./);
  assert.match(app, /Enter ADMIN_SECRET before editing items\./);
  ...
  assert.match(app, /Nothing is loaded until the token is accepted\./);
  assert.match(app, /Sources & sync/);
  assert.match(app, /adminStatus \? "Refresh" : "Unlock"/);
  assert.match(app, /Recall Inbox/);
  assert.match(app, /Enter ADMIN_SECRET to load private saved items and manage sources\./);
